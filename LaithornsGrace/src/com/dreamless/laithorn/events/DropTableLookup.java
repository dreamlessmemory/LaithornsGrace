...
import com.dreamless.laithorn.DataHandler;
...
public class DropTableLookup {
	...
	private static HashMap<EntityType, MobDropTableEntry> mobDropTables = new HashMap<EntityType, MobDropTableEntry>();
	...
	public static void addMobDropTable(EntityType type, MobDropTableEntry entry){
		mobDropTables.put(type, entry);
	}
	...
	public static ItemStack dropMobItems(LivingEntity entity) {
		...
		if (data != null) {
			try {
				MobDropTableEntry entry = mobDropTables.get(entity.getType());
				if (entry == null) {
				...
				if(Math.random() <= entry.getDropChance()) { //Successful roll
					return CustomRecipes.fragmentItem(getLevel(data), entry.getBaseType(), entry.getTags());
				}
